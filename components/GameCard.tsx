...
export const GameCard: React.FC<GameCardProps> = ({
  ...
  if (!card) {
    return (
      <div
        className={`w-full aspect-[4/3] max-w-[600px] rounded-xl sm:rounded-2xl flex items-center justify-center ${theme === "classic" ? "bg-gray-100" : "bg-wnrs-darkgrey"} opacity-50`}
      >
  ...
  return (
    <div
      className="relative w-full flex items-center justify-center perspective-1000 cursor-pointer group"
      onClick={onFlip}
    >
      <motion.div
        key={card.id}
      ...
      >
        ...
        >
          ...
          <p className="mt-1 sm:mt-2 text-[10px] sm:text-xs tracking-widest uppercase opacity-80">
            Prawn Edition
          </p>
          {card.category && (
        ...
        >
          ...
          <div className="flex-1 flex items-center justify-center w-full overflow-hidden px-1 sm:px-2 md:px-4">
            <h2
              className={`${getFontSizeClass(card.text.length)} font-bold leading-snug select-none w-full break-words`}
            >
              {card.text}
            </h2>
          </div>
